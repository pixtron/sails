...
function tryToParseJSON (json) {
  ...
  try {
    // See http://stackoverflow.com/a/3143231/486547 and waterline-criteria
    var xIsoDate = /(\d{4}-[01]\d-[0-3]\dT[0-2]\d:[0-5]\d:[0-5]\d\.\d+([+-][0-2]\d:[0-5]\d|Z))|(\d{4}-[01]\d-[0-3]\dT[0-2]\d:[0-5]\d:[0-5]\d([+-][0-2]\d:[0-5]\d|Z))|(\d{4}-[01]\d-[0-3]\dT[0-2]\d:[0-5]\d([+-][0-2]\d:[0-5]\d|Z))/;

    function parseJsonDate(key, val) {
      if(!_.isString(val)) return val;

      return val.match(xIsoDate) ? new Date(val) : val;
    }

    return JSON.parse(json, parseJsonDate);
  }
  catch (e) { return e; }
}
